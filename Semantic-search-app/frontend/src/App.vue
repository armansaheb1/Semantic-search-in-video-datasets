<template>
  <div class="container">
    ...
    <div class="input-group">
      ...
      <input type="number" id="no-of-records" v-model="noOfRecords" @focus="showNoOfRecordsTooltip = true"
        @input="showNoOfRecordsTooltip = false" />
      <div class="tooltip" v-if="showNoOfRecordsTooltip">Enter the number of records you want.</div>
    </div>

    <div class="input-group">
      <div class="tooltip" v-if="showMinimumDistanceTooltip">Enter distance between (0,1]. More closer to 0 means better
        results.</div>
    </div>
    <div>
      ...
      <div class="card-container">
        <div v-for="(result, index) in paginatedResults" :key="index" class="card">
          ...
          <div class="video-preview">
            <video controls>
              <source :src="result.video_id + `#t=${parseInt(result.starttime) * 60}`">
            </video>
          </div>
          <div class="card-content">
            ...
            <a :href="result.video_id" class="video-link">Watch Video</a>
          </div>
        </div>
      </div>
    </div>
...
input[type="number"] {
  ...
  /* Add a smooth transition on border color change */
}
...
.tooltip {
  position: absolute;
  top: -30px;
  /* Adjust the position of the tooltip above the input field */
  left: 50%;
  ...
  pointer-events: none;
  /* Prevent the tooltip from affecting the input interaction */
}
